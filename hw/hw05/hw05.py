...
def store_digits(n):
    """Stores the digits of a positive number n in a linked list.0
...
def generate_paths(t, x):
    ...
    for branch in t.branches:
        for i in generate_paths(branch, x):
            yield [t.label] + i
...
def deep_map(f, link):
    ...
    if link.rest is Link.empty:
        if isinstance(link.first, Link):
            x.first = deep_map(f, link.first)
        else:
            x.first = f(link.first)
            x.rest = Link.empty
    elif isinstance(link.first, Link):
        x.first = deep_map(f, link.first)
        x.rest = deep_map(f, link.rest)
    else:
        x.first = f(link.first)
        x.rest = deep_map(f, link.rest)
    return x
