...
(define (enumerate s)
  ; BEGIN PROBLEM 16
    (define (make old ind)
      (if (null? (cdr old))
          (cons (cons ind (cons (car old) nil)) nil)
          (append (cons (cons ind (cons (car old) nil)) nil) 
                   (make (cdr old) (+ ind 1)))))
  (if (null? s)
    nil
    (make s 0))
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               ...
               (body   (cddr expr)))
           ; BEGIN PROBLEM 18
           (append (list form) (append (list params) (map let-to-lambda body)))
           ; END PROBLEM 18
           ))
        ((let? expr)
        ...
        (else
         ; BEGIN PROBLEM 18
         (append (cons (car expr) nil) (map let-to-lambda (cdr expr)))
         ; END PROBLEM 18
         )))
