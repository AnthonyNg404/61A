...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(procedure)  # As hint from piazza
        if isinstance(procedure, MacroProcedure):
            return scheme_eval(procedure.apply_macro(rest, env), env)  # Eval marco procedure
        return scheme_apply(procedure, rest.map(lambda x: scheme_eval(x, env)), env)  # Eval all the expression with MAP method
        # END PROBLEM 4
...
def eval_all(expressions, env, tail=False):
    ...
    while expressions.rest is not nil:
        ...
        expressions = expressions.rest  # Eval all the expression with while method
...
class Frame(object):
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 2
        self.bindings[symbol] = value  # Create pairs in dictionary
        # END PROBLEM 2
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            return self.bindings[symbol]  # Return things in local frame
        elif symbol not in self.bindings and self.parent is not None:
            return self.parent.lookup(symbol)  # Find and return things in parent frames
        # END PROBLEM 2
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            ...
            formals, vals = formals.rest, vals.rest  # Create all the bindings in child frame
        return new_frame
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        if not self.use_env:
            try:
                return self.fn(*python_args)  # Return result if local frame has enough operands
            except TypeError:
                raise SchemeError
        else:
            return self.fn(*python_args, env)  # Return result with environment
        # END PROBLEM 3
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        return self.env.make_child_frame(self.formals, args)  # Create new frame for Lambda procedure
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_numberp(expressions.rest.first):
            env.define(target, expressions.rest.first)  # Binding symbol with number
        else:
            env.define(target, scheme_eval(expressions.rest.first, env))  # Binding symbol with result of procedure
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        formals = target.rest
        env.define(target.first, LambdaProcedure(formals, expressions.rest, env))  # Binding symbol with Lambda procedure
        return target.first
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    return expressions.first  # Return things behind quote
    # END PROBLEM 6
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    return LambdaProcedure(expressions.first, expressions.rest, env)  # Create object of Lambda procedure
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            return scheme_eval(expressions.first, env, True)  # Return last True object in list
        last = scheme_eval(expressions.first, env)
        if scheme_falsep(last):
            return False  # Return False when meets first False object
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            return scheme_eval(expressions.first, env, True)  # Return last object in list
        first = scheme_eval(expressions.first, env)
        if scheme_truep(first):
            return first  # Return True when meets first True object
        expressions = expressions.rest
    return False
    # END PROBLEM 12
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.rest is nil:
                return test  # Return test if no expression behind test
            return eval_all(clause.rest, env, True)  # Return eval of expression behind test
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        bindings = bindings.rest  # Pair all the bindings in let form
    return env.make_child_frame(formals, vals)  # Eval let bindings in new child frame
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 20
    check_form(expressions, 2)
    check_form(expressions.first, 0)
    check_formals(expressions.first)  # Check marco format
    macro = MacroProcedure(expressions.first.rest, expressions.rest, env)
    env.define(expressions.first.first, macro)  # Create and define marco procedure in current frame
    return expressions.first.first  # Return procedure name
    # END Problem 20
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15
    def make_call_frame(self, args, env):
        return env.make_child_frame(self.formals, args)  # Create child frame for Mu procedure
    # END PROBLEM 15
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            result = original_scheme_eval(result.expr, result.env)  # Eval expression in old ways when it is not tail expression
        return result
        # END
