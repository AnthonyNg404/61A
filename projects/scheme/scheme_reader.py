...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 1
        return read_tail(src)  # Read the rest
        # END PROBLEM 1
    elif val in quotes:
        # BEGIN PROBLEM 6
        first = scheme_read(src)
        return Pair(quotes[val], Pair(first, nil))  # Pair things behind quote expression
        # END PROBLEM 6
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            return Pair(scheme_read(src), read_tail(src))  # Finish the pair procedure
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
